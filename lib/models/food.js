...
function updateAndReturnResource(food, id) {
    if (food.name) {
        return database.raw('UPDATE foods SET name = ? WHERE id = ? RETURNING id, name, calories', [food.name, id])
            .then(function(response) {
                return response.rows[0]
            })
    } else {
        return database.raw('UPDATE foods SET calories = ? WHERE id = ? RETURNING id, name, calories', [food.calories, id])
            .then(function(response) {
                return response.rows[0]
            })
    }
